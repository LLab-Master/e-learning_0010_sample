...
def search_stock1(request):
    qs1 = Stock.objects.filter(product__name='鉛筆')
    print(qs1)
    stocks = []
    for qs in qs1:
        stocks.append({'name':qs.product.name, 'count':qs.count})

    return render(request, 'search_stock1.html', {'stocks':stocks})

def search_stock2(request):
    qs1 = Stock.objects.filter(product__name='鉛筆')
    print(qs1)
    stocks = []
    for qs in qs1:
        stocks.append({'product':qs.product, 'count':qs.count})

    return render(request, 'search_stock2.html', {'stocks':stocks})
